...
public class Risa extends AbstractSpriterMonster {

    public static final String ID = "Risa";
...
//        super(NAME, "Anon", 1000, -8.0F, 0.0F, 360.0F, 240.0F, null, x, y);
    ...
    public void takeTurn() {
        switch (this.nextMove) {
            case 0:
                AbstractDungeon.actionManager.addToBottom(new MakeTempCardInDiscardAction(new Roselia(), 1));
                Iterator var3 = AbstractDungeon.getCurrRoom().monsters.monsters.iterator();

                while(var3.hasNext()) {
                    AbstractMonster m2 = (AbstractMonster)var3.next();
                    if (m2.id.equals(Yukina.ID)) {
                        AbstractDungeon.actionManager.addToBottom(new GainBlockAction(m2,  30));
                    }
                }
                break;
            case 1:
    ...
    protected void getMove(int num) {
        if (num < 50) {
            this.setMove((byte) 0, Intent.DEFEND, 0, 0, true);
        }else {
            this.setMove((byte) 1, Intent.DEBUFF, 0, 0, true);
        }
        this.createIntent();
    }
