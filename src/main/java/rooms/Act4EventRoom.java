...
public class Act4EventRoom extends AbstractRoom {

    MapRoomNode currNode;
    MapRoomNode node;
    public Act4EventRoom(MapRoomNode enemyNode) {
...
//        this.node = new MapRoomNode(this.currNode.x, this.currNode.y);
        this.node.setRoom(new EventRoom() {
            public void onPlayerEntry() {
                AbstractDungeon.overlayMenu.proceedButton.hide();
                this.event = new AnondeterminationEvent();
                this.event.onEnterRoom();
            }
        });
        this.node.room.setMapImg(ImageMaster.loadImage("img/UI/special.png"), ImageMaster.loadImage("img/UI/special.png"));
