...
public class AnonEndingEffect extends AbstractGameEffect {
    ...
    public AnonEndingEffect() {
        AbstractDungeon.overlayMenu.hideBlackScreen();
        this.duration = this.startingDuration = 10f;
...
//        CardCrawlGame.sound.play(AudioPatch.Sound_Last_Word);
        ...
        new Thread(() -> {
            try {
                videoPlayer.play(Gdx.files.internal("movie/mygoEnding.webm"));
...
//        AbstractDungeon.effectsQueue.add(new LastWordScreenEffect());
    ...
    public void update() {
        ...
        if (startPlay) {
            videoPlayer.update();
            expandRate = MathUtils.lerp(expandRate, 1f, Gdx.graphics.getDeltaTime() * 5.0F);
            if (expandRate > 1f) expandRate = 1f;
        }

        if (!inTail && (this.startingDuration - this.duration >= 100f)) {
            inTail = true;
            AbstractDungeon.overlayMenu.hideBlackScreen();
        }
        if (inTail) {
