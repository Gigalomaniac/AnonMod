...
public class character3DHelper
{
  ...
  private void init() {
    ...
    this.psb = new PolygonSpriteBatch();
        ...
        this.modelController = new ModelController("img/mika/mika.g3dj", 0.0F, 0.0F, 0.0F, "bone_root|CH0069_Normal_Idle");
    ...
    this.modelController.getTransForm().rotate(0.0F, 0.0F, 1.0F, 5.0F);
      this.modelController.getTransForm().rotate(0, 1, 0, 25);
  ...
  public void render(SpriteBatch spriteBatch, boolean flipHorizontal) {
   if(this.modelController == null)
    ...
    this.psb.begin();
//    System.out.println(AbstractDungeon.player.hb.x);
//      System.out.println(AbstractDungeon.player.hb.y);
    this.psb.draw(this.region, AbstractDungeon.player.hb.x*0.15f, AbstractDungeon.player.hb.y*0.2f,
...
public void queueAnimaItem(List<AnimaItem> items, boolean clearQueue) { queueAnimaItem(items, clearQueue, 1); }
    ...
    public boolean isAttacking() {
        ...
        return ((SkinSelectScreen.Inst.index == 0 && anima.equals(this.modelController.getAnimaId(MikaAnimaItem.Normal_Attack_Start.get()))));
    }
}
