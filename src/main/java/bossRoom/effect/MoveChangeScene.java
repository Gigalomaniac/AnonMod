...
public class MoveChangeScene extends AbstractChangeSceneEffect{
  ...
  public MoveChangeScene(int code) {
  ...
  public MoveChangeScene(Texture img) {
        this.color = Color.WHITE.cpy();
        this.renderBehind = true;
        this.img = img;
        this.img2 = ImageMaster.loadImage("img/vfx/Mask.png");
...
//                    }
    ...
    public void render(SpriteBatch sb) {
        ...
        sb.setColor(Color.WHITE.toFloatBits());
            float imgAspectRatio = (float) img.getHeight() / (float) img.getWidth();
            float newHeight = Settings.WIDTH * imgAspectRatio;
        if (this.timer >= -4.0F) {
            sb.draw(this.img, 0.0F, (float) (-180.0F* Math.pow(this.timer,2)), Settings.WIDTH, newHeight);
        } else {
//             sb.draw(this.img, 0.0F, -1000.0F, Settings.WIDTH, newHeight);
        }
    }
