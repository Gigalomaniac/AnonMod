...
void main() {
            float time = u_time;
        	//vec2 resolution = vec2(1334., 750.);
        		//
        	float xpart = v_texCoord.x;
        	float ypart = v_texCoord.y;
        	//
        	float clip = 0.0;
        	...
        	//
        	// sparks
        	float sparkGridSize = 40.0;
        	vec2 sparkCoord = gl_FragCoord.xy - vec2(0.,190.0*realTime);
